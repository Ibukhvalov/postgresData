...
class Santa:
    ...
    def show_emails(self):
        print("функция для показа текущих писем для сконкретного санты по self.nickname класть в self.emails")

        self.emails = [
            {"ID": "123456", "Topic": "cat", "Description": "cute small kitten"},
            {"ID": "654321", "Topic": "ball", "Description": "i would like to play football, but i haven't a ball"},
        ]

        for email in self.emails:
            self.tree.insert("", "end", values=(email["ID"], email["Topic"], email["Description"]))
    ...
    def open_information_about_child(self, author_id):
        ...
        tk.Label(child_window, text = child_info["date_of_birth"]).grid(row=1, column=1, padx=10, pady=5, sticky='we')
    ...
    def get_information_about_child(self, author_id, child_info):
        print("функция для получения информации о ребенке по author_id = ", author_id)

        child_info.update({
            "full_name": "ivan bukhvalov",
            "date_of_birth": "29-01-2006",
            "postcode": "123456",
            "number_of_good_deeds": 100,
            "number_of_misdeeds": 0,
        })
